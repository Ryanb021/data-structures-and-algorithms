...
const findHappiness = (arr) => {
  return arr.filter(string => { if (string.includes(':)')) { return string } });
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(number => number.replace(/[() -]/g, ""));
};
...
const onlyOddChars = (str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    if (i % 2 !== 0) {
      oddsArray.push(str[i]);
    }
  }
  let combined = oddsArray.join('');
  return combined;
};
...
const allHappy = (arr) => {
  ...
  let lengthCheck = [];
  lengthCheck = arr.filter(item => { if (item.includes(':)')) { return item} });

  if (lengthCheck.length === originalArray.length) {
    return true;
  } else {
    return false;
  }
