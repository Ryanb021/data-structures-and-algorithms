...
const addTwo = (arr) => {
  let addTwoArray = arr.map(numbers => numbers + 2);
  return addTwoArray;
};
...
const typeNum = (arr) => {
  let onlyNumbersArray = arr.filter(items => {
...
const containsAnd = (arr) => {
  let containsAndArray = arr.filter(strings => {
...
const oddValues = (arr) => {
  let oddValuesArray = arr.filter(integers => integers % 2 === 1);
...
const notInFirstArray = (forbiddenValues, arr) => {
  let notInFirstArrayList = arr.filter((input => !forbiddenValues.includes(input)));
  return notInFirstArrayList;
};
