...
function transformToLis(obj) {
  ...
  entries.map(entry => {
    ...
    let li = '';
    entry.reduce((prev, curr) => {
      let prevText = prev;
      let nextText = curr;

      if (prevText !== undefined) {
        li = '<li>' + prevText + ': ' + nextText + '</li>';
      }
    });
    arrOfEl.push(li);
  });
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  filteredArr = input.map(arr => arr.filter(num => {
    if (num % 5 === 0 && typeof num === 'number' && !isNaN(parseInt(num))) {
      return num;
    }
  }));

  masterArr = filteredArr.map(item => item.map(num => Math.pow(2, num)));
...
let findShortest = (data) => {
  let reducedItem = data.reduce((prev, curr) => {
    if (prev !== undefined) {
      if (parseInt(prev.height) > parseInt(curr.height)) {
        prev = curr;
      } else {
