...
const screenForNames = (arr) => {
  let filteredArray = [];
  arr.forEach(name => {
    if ((/^(Mr\.|Mrs\.|Ms\.|Dr\.)\s?[a-zA-Z]+$/).test(name)) {
...
let biggerThanLuke = (arr) => {
  const luke = arr.filter(trooper => trooper.name === 'Luke Skywalker');

  let biggerMassArray = [];

  arr.forEach(trooper => {
    if (parseInt(trooper.mass) > parseInt(luke[0].mass)) {

      biggerMassArray.push(trooper.name);
    }
  });
  let combinedArray = biggerMassArray.join(' - ');
...
const detectTicTacToeWin = (board) => {
  ...
  let checkDiagonals = () => {
    let diagnolLTR = board[0][0] + board[1][1] + board[2][2];
    let diagnolRTL = board[0][2] + board[1][1] + board[2][0];

    console.log('rtl', diagnolRTL)
    console.log('ltr', diagnolLTR)

    if (diagnolLTR === 'XXX' || diagnolLTR === 'OOO') {
      return true;
    }

    if (diagnolRTL === 'XXX' || diagnolRTL === 'OOO') {
      return true;
    }
