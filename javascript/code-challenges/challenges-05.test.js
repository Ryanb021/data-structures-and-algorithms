...
const toLastNames = people => {
  let fullNamesArray = people.map(names => names.firstName + " " + names.lastName);
...
const addValues = (arr) => {
  return arr.reduce((numbers, addCurrentValue) => numbers += addCurrentValue, 0);
};
...
const addPurchases = (arr) => {
  return arr.reduce((input, item) => input + item.purchasePrice, 0);
};
...
const countNumberOfElements = (arr) => {
  return arr.reduce((input, item, index) => input + index -1, 0);
};
...
const reversedString = (str) => {
  ...
  return charactersArray.reduce((letters, characters) => characters + letters, '');
};
